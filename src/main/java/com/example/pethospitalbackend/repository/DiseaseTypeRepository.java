...
public interface DiseaseTypeRepository extends JpaRepository<DiseaseType,Integer> {
    ...
    Integer getId(String type, String name);
    ...
    @Transactional
    @Modifying
    @Query(value = "delete from disease_type where type =?1 and name = ?2",nativeQuery = true)
    void deleteDiseaseType(String type, String name);
}
